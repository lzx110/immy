...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    function init() {
      ...
      $topPanelScroller = $("<div class='slick-top-panel-scroller ui-state-default' style='overflow:hidden;position:relative;height:28px' />").appendTo($container);
      $topPanel = $("<div class='slick-top-panel'style='margin-top:4px;' />")/*.width($topPanelScroller.width())*/.appendTo($topPanelScroller);
      
      //written by Shaman   topPanel 中增加两个自定义button
      if (options.showTopPanel&&options.topPanelButtonAttr.length) {
		  for (var i = 0; i < options.topPanelButtonAttr.length; i++) {
			  var permissionName = options.topPanelButtonAttr[i].permission;
				  ...
				  $.ajax({
					  ...
					  success: function(data) {
						  ...
						  if (permission || permissionName == null) {
							  if (options.topPanelButtonAttr[i].src != undefined) {
								  $img = $("<img border='0' style='margin:2px;cursor:pointer'></img>");
								  $img.attr("src",options.topPanelButtonAttr[i].src);
								  $topPanelButton = $("<span class='btn-mini' style='display:inline;width:20px;float:left'></span>").appendTo($topPanel);
								  $topPanelButton.attr("alt",options.topPanelButtonAttr[i].alt);
								  $topPanelButton.append($img);
//							  $topPanelButton.addClass(options.topPanelButtonAttr[i].src);
//							  $topPanelImg = $("<span >").appendTo($topPanelButton);
								  $topPanelButton.attr("name",options.topPanelButtonAttr[i].name)
								  //.attr("onclick",options.topPanelButtonAttr[i].onclick)
								  .attr("title",options.topPanelButtonAttr[i].title)
								  .addClass(options.topPanelButtonAttr[i].className);
							  } else {
								  $a = $("<a href='javascript:void(0);'></a>");
								  $a.text(options.topPanelButtonAttr[i].text);
								  $topPanelButton = $("<span class='btn-mini' style='display:inline;width:28px;float:left'></span>").appendTo($topPanel);
								  $topPanelButton.attr("alt",options.topPanelButtonAttr[i].alt);
								  $topPanelButton.append($a);
								  $topPanelButton.attr("name",options.topPanelButtonAttr[i].name)
								  .attr("title",options.topPanelButtonAttr[i].title)
								  .addClass(options.topPanelButtonAttr[i].className);
							  }
//							  $topPanelImg.attr("alt",options.topPanelButtonAttr[i].alt)
//							  .addClass(options.topPanelButtonAttr[i].src);
							  var click=options.topPanelButtonAttr[i].onclick;
							  $topPanelButton.bind("click",function(){
								  eval(click);
							  });
						  }
				...
				$.ajax({
					...
					success : function(result) {
						if(null==result.tableNameDepict){
							...
							tableName = result.tableNameDepict.replace((/,/ig),"<br>");
						}
					}
				});
    ...
    // 拼装分页 html代码   written by Shaman
    function constructPageUI() {
    	var pageContainerId = $container.attr("id") + "_pageUIId";
    	if($('#'+pageContainerId).length > 0) {
    	  return;
      }
  	  $pageContainer = $("<div style='height:26px;' class='slick-pager grid-position-style ' id=" + pageContainerId + "> </div>");
  	  $pageContainer.width($container.width());
  	  var $items = $("<span class='slick-pager-nav' style='padding:0px;margin-top:2px'></span>").appendTo($pageContainer);
        var currentPageId = pageContainerId + '_currentPageId';
  	  var totalPagesId = pageContainerId + '_totalPagesId';
  	 $("<span class='span-page'>第<span id="+ currentPageId + ">1</span>页</span>").appendTo($items);
  	 	 $("<span class='span-page'>/共<span id="+ totalPagesId + "></span>页</span>").appendTo($items);

  	  $(" <span> <a class='page-change'>首页</a></span>")
  	     .click(function(){
  	    	 changePage(currentPageId,totalPagesId,'first');
  	    	 range = [];
  			 getSelectionModel().setSelectedRanges(range);
        }).appendTo($items);
       
        $("<a class='page-change'>上一页</a>")
        	  .click(function(){
        		  changePage(currentPageId,totalPagesId,'pre');
        		  range = [];
        		  getSelectionModel().setSelectedRanges(range);
        	  }).appendTo($items);    
        $("<a class='page-change'>下一页</a>")
        	  .click(function(){
        		  changePage(currentPageId,totalPagesId,'next');
        		  range = [];
        		  getSelectionModel().setSelectedRanges(range);
        	  }).appendTo($items);
        $("<a class='page-change'>末页</a>")
        	  .click(function(){
        		  changePage(currentPageId,totalPagesId,'last');
        		  range = [];
        		  getSelectionModel().setSelectedRanges(range);
        	  }).appendTo($items);
        var totalCountId = pageContainerId + '_totalCountId';
        $("<span class='span-page'>共有<span id="+ totalCountId + "></span>条数据</span>").appendTo($items);
        
        
        $pageContainer.insertAfter($container);
      }
    function changePage(currentPageId,totalPagesId,type) {
   	  var totalPages = parseInt($('#'+totalPagesId).text());
   	  
   	  if(totalPages == 0){
   		  return ;
   	  }
        var currentPageNum = parseInt($('#'+currentPageId).text());
		  if ('first' == type) {
			$('#'+currentPageId).text(1);
		  }else if ('pre' == type) {
			  if (currentPageNum > 1) {
				$('#'+currentPageId).text(currentPageNum-1);
			  }
		  }else if ('next' == type){
    ...
    function getCurrentPage(){
    	return $('#' + $container.attr("id") + "_pageUIId_currentPageId").text();
    }
    function getTotalPages(totalPagesId) {
    	return $('#' + $container.attr("id") + "_pageUIId_totalPagesId").text();
    }
    function setTotalPages(page) {
    	$('#' + $container.attr("id") + "_pageUIId_totalPagesId").text(page);
    } 
    function setTotalCount(count) {
    	$('#' + $container.attr("id") + "_pageUIId_totalCountId").text(count);
    } 
    ...
    function setActiveCellInternal(newCell, editMode) {
      ...
      if (activeCellNode != null) {
        ...
        if(!columns[activeCell].selectable && !(typeof columns[activeCell].selectable === "undefined"))
        	$(activeCellNode).removeClass("active");
